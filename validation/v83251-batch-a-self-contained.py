...
REPO=pathlib.Path.cwd();TARGET='125a6f60c94eda4c455323a171006cac613eb903';SEM='QCEvidenceExtractorV5AW -> QCSemanticCoreV122';SEALED=REPO/'validation/v83251-v1-sealed';WORK=pathlib.Path('/tmp/v83251-batch-a-authority');ROOT='validation/v83251-v1-sealed-runtime'
ATTEMPT=SEALED/'V8_3_251_BATCH_A_ATTEMPT_MARKER.json';FIRST=SEALED/'V8_3_251_SEALED_FIRST_RUN_MARKER.json';RESULT=SEALED/'V8_3_251_BATCH_A_RESULT_V1.json';STATUS=SEALED/'V8_3_251_BATCH_A_STATUS_V1.json'
...
def status(**kw):
 d={'candidate':'V8.3.251','phase':'sealed-batch-a-first-run','run_id':os.environ.get('GITHUB_RUN_ID'),'run_attempt':os.environ.get('GITHUB_RUN_ATTEMPT'),'validated_development_head_sha':TARGET,'semantic_authority':SEM,'batch_b_executed':False,'v250_sealed_rerun':False,'v251_batch_a_rerun':False,'step_111_authorized':False,'production_authorized':False};d.update(kw);STATUS.write_text(json.dumps(d,indent=2)+'\n');return d
def checkpoint():
 z=REPO/'PSC_V8_3_251_V1_BATCH_A_CHECKPOINT.zip'
 if z.exists():z.unlink()
 with zipfile.ZipFile(z,'w',zipfile.ZIP_DEFLATED) as q:
  for n in ['V8_3_251_BATCH_A_RESULT_V1.json','V8_3_251_BATCH_A_STATUS_V1.json','V8_3_251_BATCH_A_ATTEMPT_MARKER.json','V8_3_251_SEALED_FIRST_RUN_MARKER.json','V8_3_251_SEALED_AUTHORITY_V1.json','V8_3_251_PRESEAL_DIVERSITY_AUDIT_V1.json','V8_3_251_SEALED_SELECTION_V1.json','V8_3_251_SEALED_FIXTURE_V1.json','V8_3_251_INDEPENDENT_GOLD_V1.json','V8_3_251_SEALED_MEMBERSHIP_V1.json','V8_3_251_TRANSPORT_VERIFICATION_V1.json','V8_3_251_STATIC_SEALED_PREFLIGHT_V1.json']:
   p=SEALED/n
   if p.exists():q.write(p,p.name)
 (REPO/'PSC_V8_3_251_V1_BATCH_A_CHECKPOINT_SHA256.txt').write_text(fsha(z)+'  '+z.name+'\n')
st=None
try:
 ...
 A=json.loads((SEALED/'V8_3_251_SEALED_AUTHORITY_V1.json').read_text());pf=json.loads((SEALED/'V8_3_251_STATIC_SEALED_PREFLIGHT_V1.json').read_text());tr=json.loads((SEALED/'V8_3_251_TRANSPORT_VERIFICATION_V1.json').read_text())
 ...
 assert pf['exact_v5aw_path']=='validation/qc-evidence-extractor-v5aw-v251-v250-b-residuals.js' and pf['exact_v122_path']=='validation/psc-v83251-v250-v122-b-residuals.js'
 files={'candidate_bank':'V8_3_251_PRESEAL_CANDIDATE_BANK_V1.json','selection':'V8_3_251_SEALED_SELECTION_V1.json','fixture':'V8_3_251_SEALED_FIXTURE_V1.json','independent_gold':'V8_3_251_INDEPENDENT_GOLD_V1.json','membership':'V8_3_251_SEALED_MEMBERSHIP_V1.json','preseal_audit':'V8_3_251_PRESEAL_DIVERSITY_AUDIT_V1.json'}
 ...
 for fn in ['V8_3_251_SEALED_SELECTION_V1.json','V8_3_251_SEALED_FIXTURE_V1.json','V8_3_251_INDEPENDENT_GOLD_V1.json']:shutil.copy2(SEALED/fn,runtime/fn)
 ...
 runner=runtime/'run-v83251-batch-a.cjs';tail=r'''
if(!s.QCEvidenceExtractorV5AW)throw Error('V5AW missing');if(!s.QCSemanticCoreV122)throw Error('V122 missing');
const root='validation/v83251-v1-sealed-runtime';const F=JSON.parse(fs.readFileSync(root+'/V8_3_251_SEALED_FIXTURE_V1.json'));const S=JSON.parse(fs.readFileSync(root+'/V8_3_251_SEALED_SELECTION_V1.json'));const G=JSON.parse(fs.readFileSync(root+'/V8_3_251_INDEPENDENT_GOLD_V1.json'));const gm=new Map(G.cases.map(x=>[x.case_id,x.expected]));const ids=new Set(S.batch_a);const cases=F.cases.filter(x=>ids.has(x.case_id));if(cases.length!==30)throw Error('A cardinality '+cases.length);let passed=0;const results=[];for(const c of cases){const r=s.QCSemanticCoreV122.analyze(c.surface,c.domain);const actual={route:r.input_route.id,families:[...(r.families||[])].sort(),sequence:!!r.sequence};const g=gm.get(c.case_id);if(!g)throw Error('missing gold '+c.case_id);const expected={route:g.route,families:[...(g.families||[])].sort(),sequence:!!g.sequence};const ok=JSON.stringify(actual)===JSON.stringify(expected);if(ok)passed++;results.push({case_id:c.case_id,category:c.category,language:c.language,domain:c.domain,pass:ok,expected,actual,surface:c.surface});}const out={authority:'V8.3.251 V1 SEALED BATCH A FIRST RUN',run_id:process.env.GITHUB_RUN_ID,run_attempt:process.env.GITHUB_RUN_ATTEMPT,validated_development_head_sha:'125a6f60c94eda4c455323a171006cac613eb903',semantic_authority:'QCEvidenceExtractorV5AW -> QCSemanticCoreV122',total:30,passed,failed:30-passed,results};fs.writeFileSync('V8_3_251_BATCH_A_RESULT_V1.json',JSON.stringify(out,null,2));console.log('V251 A',passed+'/30');if(passed!==30)process.exit(1);
'''
 runner.write_text(prefix+tail)
 cp=subprocess.run(['node',str(runner.relative_to(WORK))],cwd=WORK,text=True);src=WORK/'V8_3_251_BATCH_A_RESULT_V1.json'
