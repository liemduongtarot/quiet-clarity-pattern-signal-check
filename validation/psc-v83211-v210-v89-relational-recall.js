(function(global){
'use strict';
const parent=global.QCSemanticCoreV88;if(!parent)throw new Error('V8.3.211 V89 requires V88');
const stable=global.QCSemanticCoreV86;if(!stable)throw new Error('V8.3.211 V89 requires V86 stable fallback');
const extractor=global.QCEvidenceExtractorV4;if(!extractor)throw new Error('V8.3.211 V89 requires V4');
const VERSION='V8.3.211-V89-RELATIONAL-RECALL';
...
function analyze(raw,domain='other',subtopic=null){
 ...
 if(base.input_route&&base.input_route.id==='input:prediction'&&!(s.future_outcome_request&&s.future_horizon_present)){
  const fb=stable.analyze(extractor.scopeRaw(raw),domain,subtopic);return{...fb,version:VERSION,canonical_shadow:{...(fb.canonical_shadow||{}),semantic_rule_table:'V89',semantic_rule:'contained-prediction-fallback',evidence_extractor:'V4'}};
 }
 return{...base,version:VERSION,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V89',semantic_rule:'parent-preserved',evidence_extractor:'V4'}};
}
const core={...parent,version:VERSION,analyze,inputRoute:raw=>analyze(raw).input_route,schema:Object.freeze({...parent.schema,canonicalSemanticState:'v8.3.211-v89-relational-recall'})};
global.QCSemanticCoreV89=core;global.PSC_V83211=core;
