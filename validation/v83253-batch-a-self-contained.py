...
REPO=pathlib.Path.cwd();TARGET='89db04ec936af998c14d2f0451ef87270ea8e349';SEM='QCEvidenceExtractorV5AY -> QCSemanticCoreV124';SEALED=REPO/'validation/v83253-v1-sealed';WORK=pathlib.Path('/tmp/v83253-batch-a-authority');ROOT='validation/v83253-v1-sealed-runtime'
ATTEMPT=SEALED/'V8_3_253_BATCH_A_ATTEMPT_MARKER.json';FIRST=SEALED/'V8_3_253_SEALED_FIRST_RUN_MARKER.json';RESULT=SEALED/'V8_3_253_BATCH_A_RESULT_V1.json';STATUS=SEALED/'V8_3_253_BATCH_A_STATUS_V1.json'
...
try:
 ...
 runner=runtime/'run-v83253-batch-a.cjs';tail=r'''
if(!s.QCEvidenceExtractorV5AY)throw Error('V5AY missing');if(!s.QCSemanticCoreV124)throw Error('V124 missing');
const root='validation/v83253-v1-sealed-runtime';const F=JSON.parse(fs.readFileSync(root+'/V8_3_253_SEALED_FIXTURE_V1.json'));const S=JSON.parse(fs.readFileSync(root+'/V8_3_253_SEALED_SELECTION_V1.json'));const G=JSON.parse(fs.readFileSync(root+'/V8_3_253_INDEPENDENT_GOLD_V1.json'));const gm=new Map(G.cases.map(x=>[x.case_id,x.expected]));const ids=new Set(S.batch_a);const cases=F.cases.filter(x=>ids.has(x.case_id));if(cases.length!==30)throw Error('A cardinality '+cases.length);let passed=0;const results=[];for(const c of cases){const r=s.QCSemanticCoreV124.analyze(c.surface,c.domain);const actual={route:r.input_route.id,families:[...(r.families||[])].sort(),sequence:!!r.sequence};const g=gm.get(c.case_id);if(!g)throw Error('missing gold '+c.case_id);const expected={route:g.route,families:[...(g.families||[])].sort(),sequence:!!g.sequence};const ok=JSON.stringify(actual)===JSON.stringify(expected);if(ok)passed++;results.push({case_id:c.case_id,category:c.category,language:c.language,domain:c.domain,pass:ok,expected,actual,surface:c.surface});}const out={authority:'V8.3.253 V1 SEALED BATCH A FIRST RUN',run_id:process.env.GITHUB_RUN_ID,run_attempt:process.env.GITHUB_RUN_ATTEMPT,validated_development_head_sha:'89db04ec936af998c14d2f0451ef87270ea8e349',semantic_authority:'QCEvidenceExtractorV5AY -> QCSemanticCoreV124',total:30,passed,failed:30-passed,results};fs.writeFileSync('V8_3_253_BATCH_A_RESULT_V1.json',JSON.stringify(out,null,2));console.log('V252 A',passed+'/30');if(passed!==30)process.exit(1);
'''
 runner.write_text(prefix+tail)
 cp=subprocess.run(['node',str(runner.relative_to(WORK))],cwd=WORK,text=True);src=WORK/'V8_3_253_BATCH_A_RESULT_V1.json'
