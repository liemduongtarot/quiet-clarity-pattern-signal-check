...
R=pathlib.Path('.');O=R/'validation/v83221-v1-sealed'
...
assert A['validated_development_head_sha']=='0e17e6d62edf8334ea75ba70b4f28f2746dd8436'
assert A['semantic_authority']=='QCEvidenceExtractorV5Y -> QCSemanticCoreV98' and A['preseal_pass'] is True
payload={};details={}
...
with zipfile.ZipFile(buf,'w',zipfile.ZIP_DEFLATED) as z:
 ...
 z.writestr('V8_3_221_SEALED_AUTHORITY_V1.json',(O/'V8_3_221_SEALED_AUTHORITY_V1.json').read_bytes())
bundle=buf.getvalue();bp=O/'V8_3_221_FROZEN_TRANSPORT_BUNDLE_V1.zip';bp.write_bytes(bundle);bsha=raw(bundle)
(O/'V8_3_221_FROZEN_TRANSPORT_BUNDLE_V1_SHA256.txt').write_text(bsha+'  '+bp.name+'\n')
with zipfile.ZipFile(io.BytesIO(bundle)) as z:
 assert z.testzip() is None
 assert set(z.namelist())==set(payload)|{'V8_3_221_SEALED_AUTHORITY_V1.json'}
 for k,n in FILES.items():assert csha(json.loads(z.read(n).decode()))==A['hashes'][k]
result={'candidate':'V8.3.221','phase':'transport-only','validated_development_head_sha':A['validated_development_head_sha'],'semantic_authority':A['semantic_authority'],'frozen_hashes':details,'bundle_sha256':bsha,'zip_integrity':True,'bundle_content_pure':True,'semantic_runtime_executed':False,'semantic_authority_loaded':False,'batch_a_executed':False,'batch_b_executed':False,'v217_sealed_rerun':False,'v218_sealed_rerun':False,'v219_sealed_rerun':False,'v220_sealed_rerun':False,'pass':True}
res=O/'V8_3_221_TRANSPORT_VERIFICATION_V1.json';res.write_text(json.dumps(result,indent=2)+'\n')
cp=R/'PSC_V8_3_221_V1_TRANSPORT_CHECKPOINT.zip'
with zipfile.ZipFile(cp,'w',zipfile.ZIP_DEFLATED) as z:
 for p in [res,bp,O/'V8_3_221_FROZEN_TRANSPORT_BUNDLE_V1_SHA256.txt',O/'V8_3_221_SEALED_AUTHORITY_V1.json']:z.write(p,p.name)
(R/'PSC_V8_3_221_V1_TRANSPORT_CHECKPOINT_SHA256.txt').write_text(raw(cp.read_bytes())+'  '+cp.name+'\n')
