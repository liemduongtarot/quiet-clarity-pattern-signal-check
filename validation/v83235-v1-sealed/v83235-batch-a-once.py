...
REPO=pathlib.Path.cwd();TARGET='4c58dbbd7315719093e58eb01ac18acec8cdb6fd';SEM='QCEvidenceExtractorV5AK -> QCSemanticCoreV110';SEALED=REPO/'validation/v83235-v1-sealed';WORK=pathlib.Path('/tmp/v83235-batch-a-authority');ROOT='validation/v83235-v1-sealed-runtime'
ATTEMPT=SEALED/'V8_3_235_BATCH_A_ATTEMPT_MARKER.json';FIRST=SEALED/'V8_3_235_SEALED_FIRST_RUN_MARKER.json';RESULT=SEALED/'V8_3_235_BATCH_A_RESULT_V1.json';STATUS=SEALED/'V8_3_235_BATCH_A_STATUS_V1.json'
...
def status(**kw):
 d={'candidate':'V8.3.235','phase':'sealed-batch-a-first-run','run_id':os.environ.get('GITHUB_RUN_ID'),'run_attempt':os.environ.get('GITHUB_RUN_ATTEMPT'),'validated_development_head_sha':TARGET,'semantic_authority':SEM,'batch_b_executed':False,'v219_sealed_rerun':False,'v220_sealed_rerun':False,'v221_sealed_rerun':False,'v222_sealed_rerun':False,'v223_sealed_rerun':False,'v224_sealed_rerun':False,'v225_sealed_rerun':False,'v226_sealed_rerun':False,'v227_sealed_rerun':False,'v228_sealed_rerun':False,'v229_sealed_rerun':False,'v230_sealed_rerun':False,'v231_sealed_rerun':False,'v232_sealed_rerun':False,'v233_sealed_rerun':False,'v234_sealed_rerun':False,'v235_batch_a_rerun':False,'step_111_authorized':False,'production_authorized':False};d.update(kw);STATUS.write_text(json.dumps(d,indent=2)+'\n');return d
def checkpoint():
 z=REPO/'PSC_V8_3_235_V1_BATCH_A_CHECKPOINT.zip'
 if z.exists():z.unlink()
 with zipfile.ZipFile(z,'w',zipfile.ZIP_DEFLATED) as q:
  for n in ['V8_3_235_BATCH_A_RESULT_V1.json','V8_3_235_BATCH_A_STATUS_V1.json','V8_3_235_BATCH_A_ATTEMPT_MARKER.json','V8_3_235_SEALED_FIRST_RUN_MARKER.json','V8_3_235_SEALED_AUTHORITY_V1.json','V8_3_235_PRESEAL_DIVERSITY_AUDIT_V1.json','V8_3_235_SEALED_SELECTION_V1.json','V8_3_235_SEALED_FIXTURE_V1.json','V8_3_235_INDEPENDENT_GOLD_V1.json','V8_3_235_SEALED_MEMBERSHIP_V1.json','V8_3_235_TRANSPORT_VERIFICATION_V1.json','V8_3_235_STATIC_SEALED_PREFLIGHT_V1.json']:
   p=SEALED/n
   if p.exists():q.write(p,p.name)
 (REPO/'PSC_V8_3_235_V1_BATCH_A_CHECKPOINT_SHA256.txt').write_text(fsha(z)+'  '+z.name+'\n')
st=None
try:
 ...
 A=json.loads((SEALED/'V8_3_235_SEALED_AUTHORITY_V1.json').read_text());pf=json.loads((SEALED/'V8_3_235_STATIC_SEALED_PREFLIGHT_V1.json').read_text());tr=json.loads((SEALED/'V8_3_235_TRANSPORT_VERIFICATION_V1.json').read_text())
 ...
 files={'candidate_bank':'V8_3_235_PRESEAL_CANDIDATE_BANK_V1.json','selection':'V8_3_235_SEALED_SELECTION_V1.json','fixture':'V8_3_235_SEALED_FIXTURE_V1.json','independent_gold':'V8_3_235_INDEPENDENT_GOLD_V1.json','membership':'V8_3_235_SEALED_MEMBERSHIP_V1.json','preseal_audit':'V8_3_235_PRESEAL_DIVERSITY_AUDIT_V1.json'}
 ...
 for fn in ['V8_3_235_SEALED_SELECTION_V1.json','V8_3_235_SEALED_FIXTURE_V1.json','V8_3_235_INDEPENDENT_GOLD_V1.json']:shutil.copy2(SEALED/fn,runtime/fn)
 ...
 runner=runtime/'run-v83235-batch-a.cjs';tail=r'''
if(!s.QCSemanticCoreV65R)throw Error('V65R missing');if(!s.QCEvidenceExtractorV5AK)throw Error('V5AK missing');if(!s.QCSemanticCoreV110)throw Error('V110 missing');
const root='validation/v83235-v1-sealed-runtime';const F=JSON.parse(fs.readFileSync(root+'/V8_3_235_SEALED_FIXTURE_V1.json'));const S=JSON.parse(fs.readFileSync(root+'/V8_3_235_SEALED_SELECTION_V1.json'));const G=JSON.parse(fs.readFileSync(root+'/V8_3_235_INDEPENDENT_GOLD_V1.json'));const gm=new Map(G.cases.map(x=>[x.case_id,x.expected]));const ids=new Set(S.batch_a);const cases=F.cases.filter(x=>ids.has(x.case_id));if(cases.length!==30)throw Error('A cardinality '+cases.length);let passed=0;const results=[];for(const c of cases){const r=s.QCSemanticCoreV110.analyze(c.surface,c.domain);const actual={route:r.input_route.id,families:[...(r.families||[])].sort(),sequence:!!r.sequence};const g=gm.get(c.case_id);if(!g)throw Error('missing gold '+c.case_id);const expected={route:g.route,families:[...(g.families||[])].sort(),sequence:!!g.sequence};const ok=JSON.stringify(actual)===JSON.stringify(expected);if(ok)passed++;results.push({case_id:c.case_id,category:c.category,language:c.language,domain:c.domain,pass:ok,expected,actual,surface:c.surface});}const out={authority:'V8.3.235 V1 SEALED BATCH A FIRST RUN',run_id:process.env.GITHUB_RUN_ID,run_attempt:process.env.GITHUB_RUN_ATTEMPT,validated_development_head_sha:'4c58dbbd7315719093e58eb01ac18acec8cdb6fd',semantic_authority:'QCEvidenceExtractorV5AK -> QCSemanticCoreV110',total:30,passed,failed:30-passed,results};fs.writeFileSync('V8_3_235_BATCH_A_RESULT_V1.json',JSON.stringify(out,null,2));console.log('V235 A',passed+'/30');if(passed!==30)process.exit(1);
'''
 runner.write_text(prefix+tail);txt=runner.read_text();assert 'validation/v83234-v1-sealed-runtime' not in txt and 'V8_3_234_SEALED_FIXTURE' not in txt
 cp=subprocess.run(['node',str(runner.relative_to(WORK))],cwd=WORK,text=True);src=WORK/'V8_3_235_BATCH_A_RESULT_V1.json'
