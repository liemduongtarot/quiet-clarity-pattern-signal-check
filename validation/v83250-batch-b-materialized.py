...
REPO=pathlib.Path.cwd();TARGET='514cbcb34fbe336033c8f8858a53415371137973';SEM='QCEvidenceExtractorV5AV -> QCSemanticCoreV121';SEALED=REPO/'validation/v83250-v1-sealed';WORK=pathlib.Path('/tmp/v83250-batch-b-authority');ROOT='validation/v83250-v1-sealed-runtime'
ATTEMPT=SEALED/'V8_3_250_BATCH_B_ATTEMPT_MARKER.json';FIRST=SEALED/'V8_3_250_SEALED_FIRST_RUN_MARKER.json';RESULT=SEALED/'V8_3_250_BATCH_B_RESULT_V1.json';STATUS=SEALED/'V8_3_250_BATCH_B_STATUS_V1.json'
...
def status(**kw):
 d={'candidate':'V8.3.250','phase':'sealed-batch-b-first-run','run_id':os.environ.get('GITHUB_RUN_ID'),'run_attempt':os.environ.get('GITHUB_RUN_ATTEMPT'),'validated_development_head_sha':TARGET,'semantic_authority':SEM,'batch_a_executed':True,'v249_sealed_rerun':False,'v250_batch_b_rerun':False,'step_111_authorized':False,'production_authorized':False};d.update(kw);STATUS.write_text(json.dumps(d,indent=2)+'\n');return d
def checkpoint():
 z=REPO/'PSC_V8_3_250_V1_BATCH_B_CHECKPOINT.zip'
 if z.exists():z.unlink()
 with zipfile.ZipFile(z,'w',zipfile.ZIP_DEFLATED) as q:
  for n in ['V8_3_250_BATCH_B_RESULT_V1.json','V8_3_250_BATCH_B_STATUS_V1.json','V8_3_250_BATCH_B_ATTEMPT_MARKER.json','V8_3_250_SEALED_FIRST_RUN_MARKER.json','V8_3_250_SEALED_AUTHORITY_V1.json','V8_3_250_PRESEAL_DIVERSITY_AUDIT_V1.json','V8_3_250_SEALED_SELECTION_V1.json','V8_3_250_SEALED_FIXTURE_V1.json','V8_3_250_INDEPENDENT_GOLD_V1.json','V8_3_250_SEALED_MEMBERSHIP_V1.json','V8_3_250_TRANSPORT_VERIFICATION_V1.json','V8_3_250_STATIC_SEALED_PREFLIGHT_V1.json']:
   p=SEALED/n
   if p.exists():q.write(p,p.name)
 (REPO/'PSC_V8_3_250_V1_BATCH_B_CHECKPOINT_SHA256.txt').write_text(fsha(z)+'  '+z.name+'\n')
st=None
try:
 ...
 a_status=json.loads((SEALED/'V8_3_250_BATCH_A_STATUS_V1.json').read_text());a_result=json.loads((SEALED/'V8_3_250_BATCH_A_RESULT_V1.json').read_text());assert a_status['conclusion']=='success' and a_status['batch_a_passed']==30 and a_status['batch_a_failed']==0 and a_result['passed']==30 and a_result['failed']==0
 A=json.loads((SEALED/'V8_3_250_SEALED_AUTHORITY_V1.json').read_text());pf=json.loads((SEALED/'V8_3_250_STATIC_SEALED_PREFLIGHT_V1.json').read_text());tr=json.loads((SEALED/'V8_3_250_TRANSPORT_VERIFICATION_V1.json').read_text())
 ...
 assert pf['exact_v5av_path']=='validation/qc-evidence-extractor-v5av-v250-v249-residuals.js' and pf['exact_v121_path']=='validation/psc-v83250-v249-v121-residuals.js'
 files={'candidate_bank':'V8_3_250_PRESEAL_CANDIDATE_BANK_V1.json','selection':'V8_3_250_SEALED_SELECTION_V1.json','fixture':'V8_3_250_SEALED_FIXTURE_V1.json','independent_gold':'V8_3_250_INDEPENDENT_GOLD_V1.json','membership':'V8_3_250_SEALED_MEMBERSHIP_V1.json','preseal_audit':'V8_3_250_PRESEAL_DIVERSITY_AUDIT_V1.json'}
 ...
 for fn in ['V8_3_250_SEALED_SELECTION_V1.json','V8_3_250_SEALED_FIXTURE_V1.json','V8_3_250_INDEPENDENT_GOLD_V1.json']:shutil.copy2(SEALED/fn,runtime/fn)
 ...
 runner=runtime/'run-v83250-batch-b.cjs';tail=r'''
if(!s.QCEvidenceExtractorV5AV)throw Error('V5AV missing');if(!s.QCSemanticCoreV121)throw Error('V121 missing');
const root='validation/v83250-v1-sealed-runtime';const F=JSON.parse(fs.readFileSync(root+'/V8_3_250_SEALED_FIXTURE_V1.json'));const S=JSON.parse(fs.readFileSync(root+'/V8_3_250_SEALED_SELECTION_V1.json'));const G=JSON.parse(fs.readFileSync(root+'/V8_3_250_INDEPENDENT_GOLD_V1.json'));const gm=new Map(G.cases.map(x=>[x.case_id,x.expected]));const ids=new Set(S.batch_b);const cases=F.cases.filter(x=>ids.has(x.case_id));if(cases.length!==30)throw Error('B cardinality '+cases.length);let passed=0;const results=[];for(const c of cases){const r=s.QCSemanticCoreV121.analyze(c.surface,c.domain);const actual={route:r.input_route.id,families:[...(r.families||[])].sort(),sequence:!!r.sequence};const g=gm.get(c.case_id);if(!g)throw Error('missing gold '+c.case_id);const expected={route:g.route,families:[...(g.families||[])].sort(),sequence:!!g.sequence};const ok=JSON.stringify(actual)===JSON.stringify(expected);if(ok)passed++;results.push({case_id:c.case_id,category:c.category,language:c.language,domain:c.domain,pass:ok,expected,actual,surface:c.surface});}const out={authority:'V8.3.250 V1 SEALED BATCH B FIRST RUN',run_id:process.env.GITHUB_RUN_ID,run_attempt:process.env.GITHUB_RUN_ATTEMPT,validated_development_head_sha:'514cbcb34fbe336033c8f8858a53415371137973',semantic_authority:'QCEvidenceExtractorV5AV -> QCSemanticCoreV121',total:30,passed,failed:30-passed,results};fs.writeFileSync('V8_3_250_BATCH_B_RESULT_V1.json',JSON.stringify(out,null,2));console.log('V250 B',passed+'/30');if(passed!==30)process.exit(1);
'''
 runner.write_text(prefix+tail)
 cp=subprocess.run(['node',str(runner.relative_to(WORK))],cwd=WORK,text=True);src=WORK/'V8_3_250_BATCH_B_RESULT_V1.json'
