...
try {
  for (const tc of matrix.cases) {
    ...
    await page.waitForFunction(() => !!globalThis.QCSemanticCoreV10 && !!globalThis.PSC_V83140 && globalThis.QCSemanticCoreV4===globalThis.QCSemanticCoreV10 && !!globalThis.PSCUIAuthorityV83137, null, {timeout:30000});
...
const out={candidate:'V8.3.140',base_url:base,execution_count:1,total:results.length,passed:results.filter(x=>x.pass).length,failed:results.filter(x=>!x.pass).length,application_origin_console_errors:appConsoleErrors.length,results};
