...
REPO=pathlib.Path.cwd();TARGET='8100c9d590a95b612829e92f4a9fc859b6eddb23';SEM='QCEvidenceExtractorV5AY -> QCSemanticCoreV124';SEALED=REPO/'validation/v83255-v1-sealed';WORK=pathlib.Path('/tmp/v83255-batch-a-authority');ROOT='validation/v83255-v1-sealed-runtime'
ATTEMPT=SEALED/'V8_3_255_BATCH_A_ATTEMPT_MARKER.json';FIRST=SEALED/'V8_3_255_SEALED_FIRST_RUN_MARKER.json';RESULT=SEALED/'V8_3_255_BATCH_A_RESULT_V1.json';STATUS=SEALED/'V8_3_255_BATCH_A_STATUS_V1.json'
...
def status(**kw):
 d={'candidate':'V8.3.255','phase':'sealed-batch-a-first-run','run_id':os.environ.get('GITHUB_RUN_ID'),'run_attempt':os.environ.get('GITHUB_RUN_ATTEMPT'),'validated_development_head_sha':TARGET,'semantic_authority':SEM,'batch_b_executed':False,'v254_sealed_rerun':False,'v255_batch_a_rerun':False,'step_111_authorized':True,'production_authorized':False};d.update(kw);STATUS.write_text(json.dumps(d,indent=2)+'\n');return d
def checkpoint():
 z=REPO/'PSC_V8_3_255_V1_BATCH_A_CHECKPOINT.zip'
 if z.exists():z.unlink()
 with zipfile.ZipFile(z,'w',zipfile.ZIP_DEFLATED) as q:
  for n in ['V8_3_255_BATCH_A_RESULT_V1.json','V8_3_255_BATCH_A_STATUS_V1.json','V8_3_255_BATCH_A_ATTEMPT_MARKER.json','V8_3_255_SEALED_FIRST_RUN_MARKER.json','V8_3_255_SEALED_AUTHORITY_V1.json','V8_3_255_PRESEAL_DIVERSITY_AUDIT_V1.json','V8_3_255_SEALED_SELECTION_V1.json','V8_3_255_SEALED_FIXTURE_V1.json','V8_3_255_INDEPENDENT_GOLD_V1.json','V8_3_255_SEALED_MEMBERSHIP_V1.json','V8_3_255_TRANSPORT_VERIFICATION_V1.json','V8_3_255_STATIC_SEALED_PREFLIGHT_V1.json','V8_3_255_BATCH_A_RESERVATION_V1.json','V8_3_255_BATCH_A_CASE_PROVENANCE_V1.json','V8_3_255_BATCH_A_EXECUTION_LEDGER_V1.json','V8_3_255_SEMANTIC_FINGERPRINTS_V1.json','V8_3_255_CONTAMINATION_DUPLICATE_AUDIT_V1.json','V8_3_255_EVIDENCE_PROVENANCE_CONTRACT_COPY_V1.json','V8_3_255_SEMANTIC_SOURCE_IDENTITY_REPORT_V1.json','V8_3_255_RUNTIME_CONFIG_SCHEMA_MANIFEST_V1.json']:
   p=SEALED/n
   if p.exists():q.write(p,p.name)
 (REPO/'PSC_V8_3_255_V1_BATCH_A_CHECKPOINT_SHA256.txt').write_text(fsha(z)+'  '+z.name+'\n')
st=None
try:
 ...
 A=json.loads((SEALED/'V8_3_255_SEALED_AUTHORITY_V1.json').read_text());pf=json.loads((SEALED/'V8_3_255_STATIC_SEALED_PREFLIGHT_V1.json').read_text());tr=json.loads((SEALED/'V8_3_255_TRANSPORT_VERIFICATION_V1.json').read_text())
 ...
 identity=json.loads((SEALED/'V8_3_255_SEMANTIC_SOURCE_IDENTITY_REPORT_V1.json').read_text()); assert pf.get('semantic_source_identity_pass') is True and identity.get('pass') is True and identity.get('all_byte_identical_to_frozen_development') is True and identity.get('loaded_semantic_source_count',0)>=55
 files={'candidate_bank':'V8_3_255_PRESEAL_CANDIDATE_BANK_V1.json','selection':'V8_3_255_SEALED_SELECTION_V1.json','fixture':'V8_3_255_SEALED_FIXTURE_V1.json','independent_gold':'V8_3_255_INDEPENDENT_GOLD_V1.json','membership':'V8_3_255_SEALED_MEMBERSHIP_V1.json','preseal_audit':'V8_3_255_PRESEAL_DIVERSITY_AUDIT_V1.json'}
 ...
 for fn in ['V8_3_255_SEALED_SELECTION_V1.json','V8_3_255_SEALED_FIXTURE_V1.json','V8_3_255_INDEPENDENT_GOLD_V1.json']:shutil.copy2(SEALED/fn,runtime/fn)
 ...
 runner=runtime/'run-v83255-batch-a.cjs';tail=r'''
if(!s.QCEvidenceExtractorV5AY)throw Error('V5AY missing');if(!s.QCSemanticCoreV124)throw Error('V124 missing');
const root='validation/v83255-v1-sealed-runtime';const F=JSON.parse(fs.readFileSync(root+'/V8_3_255_SEALED_FIXTURE_V1.json'));const S=JSON.parse(fs.readFileSync(root+'/V8_3_255_SEALED_SELECTION_V1.json'));const G=JSON.parse(fs.readFileSync(root+'/V8_3_255_INDEPENDENT_GOLD_V1.json'));const gm=new Map(G.cases.map(x=>[x.case_id,x.expected]));const ids=new Set(S.batch_a);const cases=F.cases.filter(x=>ids.has(x.case_id));if(cases.length!==30)throw Error('A cardinality '+cases.length);let passed=0;const results=[];let execution_ordinal=0;for(const c of cases){execution_ordinal++;const executed_at_utc=new Date().toISOString();const r=s.QCSemanticCoreV124.analyze(c.surface,c.domain);const actual={route:r.input_route.id,families:[...(r.families||[])].sort(),sequence:!!r.sequence};const g=gm.get(c.case_id);if(!g)throw Error('missing gold '+c.case_id);const expected={route:g.route,families:[...(g.families||[])].sort(),sequence:!!g.sequence};const ok=JSON.stringify(actual)===JSON.stringify(expected);if(ok)passed++;results.push({execution_ordinal,executed_at_utc,case_id:c.case_id,category:c.category,language:c.language,domain:c.domain,pass:ok,expected,actual,surface:c.surface});}const out={authority:'V8.3.255 V1 SEALED BATCH A FIRST RUN',run_id:process.env.GITHUB_RUN_ID,run_attempt:process.env.GITHUB_RUN_ATTEMPT,validated_development_head_sha:'8100c9d590a95b612829e92f4a9fc859b6eddb23',semantic_authority:'QCEvidenceExtractorV5AY -> QCSemanticCoreV124',total:30,passed,failed:30-passed,results};fs.writeFileSync('V8_3_255_BATCH_A_RESULT_V1.json',JSON.stringify(out,null,2));console.log('V252 A',passed+'/30');if(passed!==30)process.exit(1);
'''
 runner.write_text(prefix+tail)
 cp=subprocess.run(['node',str(runner.relative_to(WORK))],cwd=WORK,text=True);src=WORK/'V8_3_255_BATCH_A_RESULT_V1.json'
 ...
 if RESULT.exists():
  ...
  reservation=json.loads((SEALED/'V8_3_255_BATCH_A_RESERVATION_V1.json').read_text());ledger=prov.new_ledger('V8.3.255','A',os.environ.get('GITHUB_RUN_ID'),os.environ.get('GITHUB_RUN_ATTEMPT'),reservation)
  matrec=json.loads((SEALED/'V8_3_255_RUNNER_MATERIALIZATION_V1.json').read_text());auth=json.loads((SEALED/'V8_3_255_SEALED_AUTHORITY_V1.json').read_text())
  ...
  for ordinal,row in enumerate(a['results'],1):
   assert row['execution_ordinal']==ordinal and row.get('executed_at_utc')
   rec=prov.finalize_case_record({'case_id':row['case_id'],'batch':'A','run_id':str(os.environ.get('GITHUB_RUN_ID')),'run_attempt':str(os.environ.get('GITHUB_RUN_ATTEMPT')),'execution_ordinal':row['execution_ordinal'],'executed_at_utc':row['executed_at_utc'],'validated_development_head_sha':TARGET,'semantic_authority':SEM,'source_runner_sha256':matrec['runner_sha256'],'candidate_bank_sha256':auth['hashes']['candidate_bank'],'selection_sha256':auth['hashes']['selection'],'fixture_sha256':auth['hashes']['fixture'],'gold_sha256':auth['hashes']['independent_gold'],'membership_sha256':auth['hashes']['membership'],'preseal_audit_sha256':auth['hashes']['preseal_audit'],'schema_contract_sha256':contract_sha,'category':row['category'],'language':row['language'],'domain':row['domain'],'surface_sha256':hashlib.sha256(row['surface'].encode()).hexdigest(),'expected':row['expected'],'actual':row['actual'],'pass':bool(row['pass']),'canonical_state_trace_ref':'V8_3_255_BATCH_A_RESULT_V1.json#'+row['case_id'],'previous_record_sha256':ledger['final_chain_sha256']})
   prov.append_case(ledger,rec);records.append(rec)
  prov.seal_ledger(ledger);ok,why=prov.verify_ledger(ledger);assert ok,why
  (SEALED/'V8_3_255_BATCH_A_CASE_PROVENANCE_V1.json').write_text(json.dumps({'candidate':'V8.3.255','batch':'A','records':records},ensure_ascii=False,indent=2)+'\n')
  (SEALED/'V8_3_255_BATCH_A_EXECUTION_LEDGER_V1.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2)+'\n')
  st=status(batch_a_attempt_consumed=True,batch_a_executed=True,batch_a_passed=a['passed'],batch_a_failed=a['failed'],semantic_runtime_executed=True,semantic_failure=bool(a['failed']),frozen_on_batch_a_failure=bool(a['failed']),case_provenance_recorded=True,execution_ledger_sealed=True,conclusion='success' if a['passed']==30 else 'failure')
