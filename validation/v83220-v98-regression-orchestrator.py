...
# Preserve the exact proven V97 regression architecture and append V5Y/V98.
...
# Materialize frozen V8.3.219 A as a separate carrier whose filename is deliberately excluded from historical *REGRESSION.json discovery.
...
cases=[{'case_id':r['case_id'],'surface':r['surface'],'domain':r['domain'],'category':r['category'],'language':r['language'],'expected':r['expected'],'v219_first_run_pass':r['pass']} for r in a['results']]
carrier={'authority':'V8.3.220 immutable carrier from frozen V8.3.219 A first run','source_run_id':a['run_id'],'cases':cases}
carrier_path=Path('validation/V8_3_220_V219_A_IMMUTABLE_CARRIER.json');carrier_path.write_text(json.dumps(carrier,ensure_ascii=False,indent=2)+'\n')
...
runner=Path('validation/run-v83220-v219-a-regression.cjs')
tail=r'''
if(!s.QCSemanticCoreV98)throw Error('QCSemanticCoreV98 missing');
const C=JSON.parse(fs.readFileSync('validation/V8_3_220_V219_A_IMMUTABLE_CARRIER.json','utf8')).cases;
