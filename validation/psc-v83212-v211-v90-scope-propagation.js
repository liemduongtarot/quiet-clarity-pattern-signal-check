(function(global){
'use strict';
const parent=global.QCSemanticCoreV89;if(!parent)throw new Error('V8.3.212 V90 requires V89');
const stable=global.QCSemanticCoreV86;if(!stable)throw new Error('V8.3.212 V90 requires V86 stable fallback');
const extractor=global.QCEvidenceExtractorV5;if(!extractor)throw new Error('V8.3.212 V90 requires V5');
const VERSION='V8.3.212-V90-SCOPE-PROPAGATION';
...
function analyze(raw,domain='other',subtopic=null){
 const scoped=extractor.scopeRaw(raw),base=parent.analyze(scoped,domain,subtopic),s=extractor.extract(scoped),rid=routeRule(s);
 if(rid)return applyRoute(base,rid,'route:'+rid);
 const fam=familyRule(s);if(fam)return applyFamily(base,fam);
 // Contain inherited routes/families against filtered evidence rather than raw distractors.
 if(base.input_route&&base.input_route.id==='input:prediction'&&!(s.future_outcome_request&&s.future_horizon_present)){
  const fb=stable.analyze(scoped,domain,subtopic);return{...fb,version:VERSION,canonical_shadow:{...(fb.canonical_shadow||{}),semantic_rule_table:'V90',semantic_rule:'contained-prediction-fallback',evidence_extractor:'V5'}};
 }
 if(base.input_route&&base.input_route.id==='input:third-party-only'&&!(s.third_party_subject&&s.hidden_internal_state&&s.observable_evidence_absent)){
  const fb=stable.analyze(scoped,domain,subtopic);return{...fb,version:VERSION,canonical_shadow:{...(fb.canonical_shadow||{}),semantic_rule_table:'V90',semantic_rule:'contained-third-party-fallback',evidence_extractor:'V5'}};
 }
 if(base.input_route&&base.input_route.id==='input:decision-request'&&!(s.delegated_decision&&s.agency_transfer_explicit&&s.choice_object_present&&!s.delegation_negated)){
  const fb=stable.analyze(scoped,domain,subtopic);return{...fb,version:VERSION,canonical_shadow:{...(fb.canonical_shadow||{}),semantic_rule_table:'V90',semantic_rule:'contained-decision-fallback',evidence_extractor:'V5'}};
 }
 return{...base,version:VERSION,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V90',semantic_rule:'scoped-parent-preserved',evidence_extractor:'V5'}};
}
const core={...parent,version:VERSION,analyze,inputRoute:raw=>analyze(raw).input_route,schema:Object.freeze({...parent.schema,canonicalSemanticState:'v8.3.212-v90-scope-propagation'})};
global.QCSemanticCoreV90=core;global.PSC_V83212=core;
