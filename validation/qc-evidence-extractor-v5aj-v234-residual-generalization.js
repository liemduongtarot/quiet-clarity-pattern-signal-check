...
function extract(raw){
 ...
 // Clarification: a real/lived episode is otherwise supplied but the user's observable endpoint response is omitted.
 ...
 // Decision: direct imperative transferring the deciding act, not mere advice/comparison.
 ...
 // Hypothetical: artificial/practice framing plus explicit non-personal/non-lived evidence.
 ...
 // Third-party: concealed/private belief requested without behavioural support.
 const thirdTarget=any(d,["someone else's concealed belief","someone else's private belief",'concealed belief','private belief','hidden belief','hidden conclusion','their mind']);
 const thirdAsk=any(d,['give me','tell me','infer','work out','state','describe']);
 const thirdNoSupport=any(d,['behaviour provides no support','behavior provides no support','no behavioural support','no behavioral support','no observable support','no words or actions support','nothing they said or did']);
 if(thirdTarget&&thirdAsk&&thirdNoSupport)o.v234_third=true;
 // Freeze: reversible/bounded starter + expanding alternatives + no initiation.
 ...
 // Ignore: unresolved central/main obligation + attention diversion into secondary/less relevant activity.
 ...
 // Slow: one bounded/finite pause + one check/review + response + closure.
 ...
 // Sequence: approach + retreat + repeated conclusion/reasoning + no new evidence.
