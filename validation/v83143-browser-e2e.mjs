...
const forensic=JSON.parse(fs.readFileSync('validation/V8_3_142_BATCH_A_FAILURE_FORENSIC_IMMUTABLE.json','utf8'));
const controls=[
...
try{
 const page=await browser.newPage();const appErrors=[];
 page.on('console',m=>{if(m.type()==='error'){appErrors.push(m.text());application_origin_console_errors++;}});
 page.on('pageerror',e=>{appErrors.push(String(e));application_origin_console_errors++;});
 await page.goto(base+'/candidate.html',{waitUntil:'networkidle',timeout:30000});
 const preflight=await page.evaluate(()=>({
  v12:globalThis.QCSemanticCoreV12?.version||null,
  v4:globalThis.QCSemanticCoreV4?.version||null,
  same:globalThis.QCSemanticCoreV4===globalThis.QCSemanticCoreV12,
  browser:globalThis.PSC_V83143_BROWSER?.version||null,
  semanticDataset:document.documentElement.dataset.pscSemanticAuthority||null,
  browserDataset:document.documentElement.dataset.pscBrowserAuthority||null
 }));
 fs.writeFileSync('V8_3_143_BROWSER_TRANSPORT_PREFLIGHT.json',JSON.stringify(preflight,null,2));
 if(preflight.v12!=='V8.3.143-SEALED-BATCH-A-CONVERGENCE'||!preflight.same||preflight.browser!=='V8.3.143-BROWSER-AUTHORITY-LOCK')throw new Error('V8.3.143 browser authority preflight failed '+JSON.stringify(preflight));
 execution_count++;
 ...
 const out={candidate:'V8.3.143',execution_count,total:results.length,passed:results.filter(x=>x.pass).length,failed:results.filter(x=>!x.pass).length,application_origin_console_errors,results};
 fs.writeFileSync('V8_3_143_BROWSER_E2E_RESULTS.json',JSON.stringify(out,null,2));
