...
"vm.runInContext(fs.readFileSync('validation/qc-evidence-extractor-v2r-v210-multilingual-aliases.js','utf8'),s,{filename:'qc-evidence-extractor-v2r-v210-multilingual-aliases.js'});\n"+
"vm.runInContext(fs.readFileSync('validation/psc-v83210-v210-semantic-rule-table-v6.js','utf8'),s,{filename:'psc-v83210-v210-semantic-rule-table-v6.js'});";
if(!src.includes(anchor))throw new Error('V209 base runner anchor missing');
src=src.replace(anchor,inject);
src=src.replaceAll("if(!s.QCSemanticCoreV78R)throw new Error('QCSemanticCoreV78R missing');","if(!s.QCSemanticCoreV84)throw new Error('QCSemanticCoreV84 missing');");
src=src.replaceAll("if(!s.QCEvidenceExtractorV1)throw new Error('QCEvidenceExtractorV1 missing');","if(!s.QCEvidenceExtractorV2R)throw new Error('QCEvidenceExtractorV2R missing');");
src=src.replaceAll('s.QCEvidenceExtractorV1.extract','s.QCEvidenceExtractorV2R.extract');
src=src.replaceAll('s.QCSemanticCoreV78R.analyze','s.QCSemanticCoreV84.analyze');
...
src=src.replaceAll('V8.3.209 DEVELOPMENT-ONLY GENERALIZATION AUDIT V1','V8.3.210 DEVELOPMENT-ONLY FRESH GENERALIZATION AUDIT V3');
src=src.replaceAll("semantic_authority:'QCSemanticCoreV78R'","semantic_authority:'QCSemanticCoreV84'");
src=src.replaceAll("extractor:'QCEvidenceExtractorV1-REVIEW-CANDIDATE'","extractor:'QCEvidenceExtractorV2R-V210-MULTILINGUAL-RELATIONAL-ALIASES'");
src=src.replaceAll('V8_3_209_DEVELOPMENT_GENERALIZATION_RESULTS_V1.json','V8_3_210_DEVELOPMENT_GENERALIZATION_RESULTS_V3.json');
const mod={exports:{}};const fn=new Function('require','module','exports','__filename','__dirname',src);fn(require,mod,mod.exports,path.resolve('generated-v83210-generalization-v3.cjs'),process.cwd());
