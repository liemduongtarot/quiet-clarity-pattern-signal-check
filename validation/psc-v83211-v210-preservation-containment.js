(function(global){
'use strict';
const parent=global.QCSemanticCoreV87;if(!parent)throw new Error('V8.3.211 V88 requires V87');
const stable=global.QCSemanticCoreV86;if(!stable)throw new Error('V8.3.211 V88 requires V86 stable fallback');
const extractor=global.QCEvidenceExtractorV3R;if(!extractor)throw new Error('V8.3.211 V88 requires V3R');
const VERSION='V8.3.211-V88-PRESERVATION-CONTAINMENT';
...
function applyRoute(base,id,rule){const input_route=frame(id,base.input_route);return{...base,version:VERSION,input_route,can_continue:input_route.action==='continue',must_stop:!!input_route.must_stop,must_redirect:!!input_route.must_redirect,families:[],sequence:false,oscillation:false,response_known:false,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V88',semantic_rule:rule,evidence_extractor:'V3R'}};}
function applyFamily(base,fam){const input_route=frame('input:self-lived',base.input_route);return{...base,version:VERSION,input_route,can_continue:true,must_stop:false,must_redirect:false,families:[...fam.families],sequence:!!fam.sequence,oscillation:!!fam.sequence,response_known:fam.families.length>0,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V88',semantic_rule:fam.rule,evidence_extractor:'V3R'}};}
function analyze(raw,domain='other',subtopic=null){
 const base=parent.analyze(raw,domain,subtopic),s=extractor.extract(raw),rid=routeRule(s);if(rid)return applyRoute(base,rid,'route:'+rid);
 const fam=familyRule(s);if(fam)return applyFamily(base,fam);
 // If a scoped target yields no prediction evidence, do not let prediction survive solely from a mechanism-neutral context sentence.
 if(base.input_route&&base.input_route.id==='input:prediction'&&!(s.future_outcome_request&&s.future_horizon_present)){
   const fb=stable.analyze(extractor.scopeRaw(raw),domain,subtopic);
   return{...fb,version:VERSION,canonical_shadow:{...(fb.canonical_shadow||{}),semantic_rule_table:'V88',semantic_rule:'contained-contextual-prediction-fallback',evidence_extractor:'V3R'}};
 }
 return{...base,version:VERSION,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V88',semantic_rule:'parent-preserved',evidence_extractor:'V3R'}};
}
const core={...parent,version:VERSION,analyze,inputRoute:raw=>analyze(raw).input_route,schema:Object.freeze({...parent.schema,canonicalSemanticState:'v8.3.211-v88-preservation-containment'})};
global.QCSemanticCoreV88=core;global.PSC_V83211=core;
