(function(global){
'use strict';
const parent=global.QCSemanticCoreV78R;if(!parent)throw new Error('SemanticRuleTableV3 requires QCSemanticCoreV78R');
const extractor=global.QCEvidenceExtractorV1R2;if(!extractor)throw new Error('SemanticRuleTableV3 requires QCEvidenceExtractorV1R2');
const VERSION='V8.3.209-SEMANTIC-RULE-TABLE-V3-REVIEW-CANDIDATE';
...
function routeRule(s){
 ...
 if(!yes(s,'delegation_negated')&&yes(s,'agency_transfer_explicit')&&yes(s,'delegated_decision'))return 'input:decision-request';
...
function analyze(raw,domain='other',subtopic=null){const base=parent.analyze(raw,domain,subtopic),slots=extractor.extract(raw),rid=routeRule(slots);
 if(rid){const input_route=frame(rid,base.input_route);return{...base,version:VERSION,input_route,can_continue:input_route.action==='continue',must_stop:!!input_route.must_stop,must_redirect:!!input_route.must_redirect,families:[],sequence:false,oscillation:false,response_known:false,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V3',semantic_rule:'route:'+rid}};}
 const fam=familyRule(slots);if(fam){const input_route=frame('input:self-lived',base.input_route);return{...base,version:VERSION,input_route,can_continue:true,must_stop:false,must_redirect:false,families:[...fam.families],sequence:!!fam.sequence,oscillation:!!fam.sequence,response_known:fam.families.length>0,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V3',semantic_rule:fam.rule}};}
 // A negated delegation statement must not inherit a false parent decision route.
 if(yes(slots,'delegation_negated')&&base.input_route&&base.input_route.id==='input:decision-request'){
   const input_route=frame('input:self-lived',base.input_route);return{...base,version:VERSION,input_route,can_continue:true,must_stop:false,must_redirect:false,families:[],sequence:false,oscillation:false,response_known:false,canonical_shadow:{...(base.canonical_shadow||{}),semantic_rule_table:'V3',semantic_rule:'negated-delegation-preservation'}};
 }
 return base;}
const core={...parent,version:VERSION,analyze,inputRoute:raw=>analyze(raw).input_route,schema:Object.freeze({...parent.schema,canonicalSemanticState:'review-candidate-semantic-rule-table-v3'})};
global.QCSemanticCoreV79RC3=core;global.PSC_V83209_RULE_TABLE_RC3=core;
