...
src=src.replaceAll("if(!s.QCSemanticCoreV78R)throw new Error('QCSemanticCoreV78R missing');","if(!s.QCSemanticCoreV83)throw new Error('QCSemanticCoreV83 missing');");
src=src.replaceAll("if(!s.QCEvidenceExtractorV1)throw new Error('QCEvidenceExtractorV1 missing');","if(!s.QCEvidenceExtractorV2)throw new Error('QCEvidenceExtractorV2 missing');");
src=src.replaceAll('s.QCEvidenceExtractorV1.extract','s.QCEvidenceExtractorV2.extract');
src=src.replaceAll('s.QCSemanticCoreV78R.analyze','s.QCSemanticCoreV83.analyze');
...
src=src.replaceAll('V8.3.209 DEVELOPMENT-ONLY GENERALIZATION AUDIT V1','V8.3.210 DEVELOPMENT-ONLY FRESH GENERALIZATION AUDIT V2');
src=src.replaceAll("semantic_authority:'QCSemanticCoreV78R'","semantic_authority:'QCSemanticCoreV83'");
src=src.replaceAll("extractor:'QCEvidenceExtractorV1-REVIEW-CANDIDATE'","extractor:'QCEvidenceExtractorV2-V210-RELATIONAL-SEMANTICS'");
src=src.replaceAll('V8_3_209_DEVELOPMENT_GENERALIZATION_RESULTS_V1.json','V8_3_210_DEVELOPMENT_GENERALIZATION_RESULTS_V2.json');
