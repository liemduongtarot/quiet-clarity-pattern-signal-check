...
REPO=pathlib.Path.cwd();TARGET='b4eb9e00fffcfea5dcd73406f1e1216810002cd2';SEM='QCEvidenceExtractorV5AM -> QCSemanticCoreV112';SEALED=REPO/'validation/v83237-v1-sealed';WORK=pathlib.Path('/tmp/v83237-batch-a-authority');ROOT='validation/v83237-v1-sealed-runtime'
ATTEMPT=SEALED/'V8_3_237_BATCH_A_ATTEMPT_MARKER.json';FIRST=SEALED/'V8_3_237_SEALED_FIRST_RUN_MARKER.json';RESULT=SEALED/'V8_3_237_BATCH_A_RESULT_V1.json';STATUS=SEALED/'V8_3_237_BATCH_A_STATUS_V1.json'
...
try:
 ...
 A=json.loads((SEALED/'V8_3_237_SEALED_AUTHORITY_V1.json').read_text());pf=json.loads((SEALED/'V8_3_237_STATIC_SEALED_PREFLIGHT_V1.json').read_text());tr=json.loads((SEALED/'V8_3_237_TRANSPORT_VERIFICATION_V1.json').read_text())
 ...
 files={'candidate_bank':'V8_3_237_PRESEAL_CANDIDATE_BANK_V1.json','selection':'V8_3_237_SEALED_SELECTION_V1.json','fixture':'V8_3_237_SEALED_FIXTURE_V1.json','independent_gold':'V8_3_237_INDEPENDENT_GOLD_V1.json','membership':'V8_3_237_SEALED_MEMBERSHIP_V1.json','preseal_audit':'V8_3_237_PRESEAL_DIVERSITY_AUDIT_V1.json'}
 ...
 for fn in ['V8_3_237_SEALED_SELECTION_V1.json','V8_3_237_SEALED_FIXTURE_V1.json','V8_3_237_INDEPENDENT_GOLD_V1.json']:shutil.copy2(SEALED/fn,runtime/fn)
 ...
 runner=runtime/'run-v83237-batch-a.cjs';tail=r'''
if(!s.QCSemanticCoreV65R)throw Error('V65R missing');if(!s.QCEvidenceExtractorV5AM)throw Error('V5AM missing');if(!s.QCSemanticCoreV112)throw Error('V112 missing');
const root='validation/v83237-v1-sealed-runtime';const F=JSON.parse(fs.readFileSync(root+'/V8_3_237_SEALED_FIXTURE_V1.json'));const S=JSON.parse(fs.readFileSync(root+'/V8_3_237_SEALED_SELECTION_V1.json'));const G=JSON.parse(fs.readFileSync(root+'/V8_3_237_INDEPENDENT_GOLD_V1.json'));const gm=new Map(G.cases.map(x=>[x.case_id,x.expected]));const ids=new Set(S.batch_a);const cases=F.cases.filter(x=>ids.has(x.case_id));if(cases.length!==30)throw Error('A cardinality '+cases.length);let passed=0;const results=[];for(const c of cases){const r=s.QCSemanticCoreV112.analyze(c.surface,c.domain);const actual={route:r.input_route.id,families:[...(r.families||[])].sort(),sequence:!!r.sequence};const g=gm.get(c.case_id);if(!g)throw Error('missing gold '+c.case_id);const expected={route:g.route,families:[...(g.families||[])].sort(),sequence:!!g.sequence};const ok=JSON.stringify(actual)===JSON.stringify(expected);if(ok)passed++;results.push({case_id:c.case_id,category:c.category,language:c.language,domain:c.domain,pass:ok,expected,actual,surface:c.surface});}const out={authority:'V8.3.237 V1 SEALED BATCH A FIRST RUN',run_id:process.env.GITHUB_RUN_ID,run_attempt:process.env.GITHUB_RUN_ATTEMPT,validated_development_head_sha:'b4eb9e00fffcfea5dcd73406f1e1216810002cd2',semantic_authority:'QCEvidenceExtractorV5AM -> QCSemanticCoreV112',total:30,passed,failed:30-passed,results};fs.writeFileSync('V8_3_237_BATCH_A_RESULT_V1.json',JSON.stringify(out,null,2));console.log('V237 A',passed+'/30');if(passed!==30)process.exit(1);
'''
 runner.write_text(prefix+tail);txt=runner.read_text();assert 'validation/v83236-v1-sealed-runtime' not in txt and 'V8_3_236_SEALED_FIXTURE' not in txt
 cp=subprocess.run(['node',str(runner.relative_to(WORK))],cwd=WORK,text=True);src=WORK/'V8_3_237_BATCH_A_RESULT_V1.json'
