...
# Capture ordinal + timestamp at the moment each semantic case begins execution.
loop_anchor="for(const c of cases){const r=s.QCSemanticCoreV124.analyze"
loop_repl="let execution_ordinal=0;for(const c of cases){execution_ordinal++;const executed_at_utc=new Date().toISOString();const r=s.QCSemanticCoreV124.analyze"
if loop_anchor not in src: raise SystemExit('semantic execution loop anchor missing')
src=src.replace(loop_anchor,loop_repl,1)
push_anchor="results.push({case_id:c.case_id"
push_repl="results.push({execution_ordinal,executed_at_utc,case_id:c.case_id"
if push_anchor not in src: raise SystemExit('semantic result push anchor missing')
src=src.replace(push_anchor,push_repl,1)
...
required=['V8_3_254_BATCH_A_CASE_PROVENANCE_V1.json','V8_3_254_BATCH_A_EXECUTION_LEDGER_V1.json','V8_3_254_BATCH_A_RESERVATION_V1.json','v83254-evidence-provenance.py','execution_ordinal','executed_at_utc','previous_record_sha256','schema_contract_sha256','QCSemanticCoreV124.analyze','238cdba6ee2d4d17bcde19de2d895a7087983345']
